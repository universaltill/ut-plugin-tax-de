...
func TestResolve_ProductClassMatrix(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			bp, ok, err := Resolve(c.orderType, c.taxCodeID, overrides)
			if err != nil {
...
// the line's own rate (pure coffee, 19% -> 19%) must answer identically
// no matter how the surrounding JSON was (re)serialized -- e.g. after an
// export/import round-trip that re-marshals the map in a different key
// order, or with different whitespace. This is the same shape as the
// equal-pair bug fixed in ut-docs#536 (a hand-created 19/19 tax code
// churning to a different code on round-trip), generalized from tax codes
// to this setting.
func TestResolve_NoOpOverrideStableAcrossReserialization(t *testing.T) {
	...
	for _, overrides := range variants {
		bp, ok, err := Resolve(OrderTypeTakeaway, "tax-coffee", overrides)
		if err != nil {
			t.Fatalf("Resolve error for %q: %v", overrides, err)
		}
		if !ok || bp != 1900 {
			t.Fatalf("Resolve(%q) = (%d, %v), want (1900, true) -- no-op override must not churn or drop", overrides, bp, ok)
		}
	}
}

func TestResolve_DineInNeverConsultsOverrides(t *testing.T) {
	// Malformed overrides JSON must never surface as an error on the
	// dine-in path -- core has already decided this plugin has nothing to
	// say before overrides are even parsed, mirroring main.go's original
	// order-type-first check.
	bp, ok, err := Resolve("", "tax-coffee", `{not json`)
	if err != nil || ok || bp != 0 {
		t.Fatalf("Resolve(dine-in, malformed overrides) = (%d, %v, %v), want (0, false, nil)", bp, ok, err)
	}
}
...
func TestResolve_OverridePresentButZeroOrNegativeTreatedAsUnset(t *testing.T) {
	for _, overrides := range []string{`{"tax-food":0}`, `{"tax-food":-100}`} {
		bp, ok, err := Resolve(OrderTypeTakeaway, "tax-food", overrides)
		if err != nil {
			t.Fatalf("Resolve error: %v", err)
		}
		if ok || bp != 0 {
...
func TestResolve_InvalidOverridesJSONOnTakeawayReturnsError(t *testing.T) {
	_, _, err := Resolve(OrderTypeTakeaway, "tax-food", `{not json`)
	if err == nil {
